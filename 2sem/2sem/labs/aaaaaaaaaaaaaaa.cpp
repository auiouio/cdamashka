...
private:
  ...
  iterator find_internal(const T &value) {
    auto current = header;
    ...
    return current && current->value == value ? iterator(current) : end();
  }
...
public:
  ...
  iterator find(const T &value) const {
    return iterator(const_cast<SkipList*>(this)->find_internal(value));
  }
  ...
  /*void erase(iterator pos) {
    ...
    for (int i = 0; i <= level && target->forward[i]; ++i) {
      if (target->forward[i]->backward[i] == target) {
    ...
    if (tail == target) {
      tail = update[0]; // The previous node at the bottom level
    }
  ...
  void erase(iterator pos) {
    ...
    // Now that all updates are done, you can safely delete the target node
    target.reset();
  }
...
int main() {
  ...
  for (float probability : {0.25f, 0.5f, 0.75f}) {
    ...
    for (int i = 0; i < numElements; ++i) {
      int a = rand() % numElements;
      skipList.erase(skipList.find(a));
    }
    end = std::chrono::high_resolution_clock::now();
